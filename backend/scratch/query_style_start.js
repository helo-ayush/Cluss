...
async function run() {
  try {
    ...
    const courses = await CourseModel.find({
      "modules.subtopics.lessonContent.blocks.body": { $regex: "STYLE START", $options: "i" }
    });

    console.log(`Found ${courses.length} courses matching "STYLE START".`);
        ...
        for (const sub of mod.subtopics) {
          if (sub.lessonContent && sub.lessonContent.blocks) {
            for (const block of sub.lessonContent.blocks) {
              if (block.body && block.body.includes("STYLE START")) {
