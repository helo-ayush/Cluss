...
async function run() {
  try {
    ...
    const courses = await CourseModel.find({
      "modules.subtopics.lessonContent.blocks.body": { $regex: "Analyze at t", $options: "i" }
    });

    console.log(`Found ${courses.length} courses matching "Analyze at t".`);
        ...
        for (const sub of mod.subtopics) {
          if (sub.lessonContent && sub.lessonContent.blocks) {
            for (const block of sub.lessonContent.blocks) {
              if (block.body && block.body.includes("Analyze at t")) {
