...
export default function DashboardTopbar({ title, usageData, onMenuClick }) {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-40 bg-[#050505]/86 px-4 py-3 md:py-4 backdrop-blur-2xl border-b border-white/5 sm:px-6 lg:left-[5.25rem] lg:px-8">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={onMenuClick}
            className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-[#1b1b1b] text-zinc-300 transition hover:bg-white hover:text-black md:hidden"
            title="Menu"
          >
          ...
          {(() => {
            ...
            return (
              <Link 
                to="/dashboard/chat" 
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full border transition md:hidden ${
                  chatActive 
                    ? 'border-white/20 bg-white text-black' 
                    : 'border-white/10 bg-[#1b1b1b] text-zinc-300 hover:bg-white hover:text-black'
                }`}
                title="AI Chat"
              >
              ...
              return (
                <Link
                  ...
                  className={`inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm font-black transition ${
                    active ? 'bg-white text-black' : 'bg-[#1b1b1b] text-zinc-300 hover:-translate-y-0.5 hover:bg-[#242424] hover:text-white'
                  }`}
                >
        ...
        <div className="flex min-w-0 items-center gap-3 md:gap-4">
          ...
          <div className="flex shrink-0 items-center gap-1.5 rounded-full bg-[#1b1b1b] border border-white/5 px-3 py-1.5">
            <Sparkles className="h-3 w-3 text-[#A3FF4F]" />
            <span className="text-[11px] font-black text-white">{usageData?.balance || 0} left</span>
          </div>

          <SignedIn>
            <Link to="/dashboard/profile" className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#202020] ring-1 ring-white/10 transition hover:ring-white/20">
              {user?.imageUrl ? (
